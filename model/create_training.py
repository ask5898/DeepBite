...
def plot_check_parameter(cfg, iqr_val, num_frames, X_true, X_med):
    ...
    if num_frames > 1000:
        ...
        plt.legend()
        plt.savefig('Filtered signal z-scored.png')
        ...
        plt.legend()
        plt.savefig('Original signal z-scored.png')
        ...
        plt.legend()
        plt.savefig('Overlayed z-scored.png')
...
def traindata_aligned(cfg, files, testfraction, num_features, savgol_filter, check_parameter):
    ...
    if check_parameter == True:
        X_true = []
        #files = ['210521_KPPTN_denv7_8DLC_dlcrnetms5_aedesJun22shuffle1_72500_full']
    ...
    for file in files:
        ...
        path_to_file = os.path.join(cfg['project_path'],"data", file, file+'-PE-seq-disc.npy')

        velocity = np.load( os.path.join(cfg['project_path'],"data", file, file+'-velocity.npy'))
        remove = np.load(os.path.join(cfg['project_path'], 'marked videos', file+'_to_remove.npy'))
        velocity = np.concatenate([velocity,[[0,0,0,0]]], axis=1)
        ...
        data = np.delete(data, rem, axis=1)
    ...
    '''for idx,feature in enumerate(data) :
        if idx in moving_avg_feat :
            feature_filtered = moving_average(feature)
            for _ in range(0, np.abs(len(moving_average(feature)) - len(filter_signal(feature)))):
                feature_filtered = np.concatenate([feature_filtered,[np.nan]])

        else :
            feature_filtered = filter_signal(feature)

        feature_filtered = interpol(feature_filtered)
        feat_mean = np.mean(feature_filtered,axis=None)
        feat_std = np.std(feature_filtered,axis=None)
        print(feat_std)
        feat_z = (feature_filtered.T - feat_mean)/feat_std
        features_norm.append(np.expand_dims(feat_z, axis=1)) '''
