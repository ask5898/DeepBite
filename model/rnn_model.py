...
class RNN_VAE(nn.Module):
    def __init__(self,TEMPORAL_WINDOW,ZDIMS,NUM_FEATURES,FUTURE_DECODER,FUTURE_STEPS, hidden_size_layer_1, 
        ...
        self.decoder_velocity = Decoder(self.seq_len,ZDIMS,NUM_FEATURES=1, hidden_size_rec=hidden_size_rec, dropout_rec=dropout_rec )
    ...
    def forward(self,seq):
        ...
        velocity = self.decoder_velocity(ins,z)
        if self.FUTURE_DECODER:
            future = self.decoder_future(ins, z)
            return prediction, future, velocity, z, mu, logvar
        else:
            return prediction, velocity, z, mu, logvar
