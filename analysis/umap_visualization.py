...
def umap_vis(file, embed, num_points,):        
    ...
    plt.grid(False)
    plt.savefig('UMAP' + file+'.png')
    plt.close()
    

def umap_label_vis(file, project_path, embed, label, n_cluster, num_points):
    fig = plt.figure(1)
    ...
    plt.grid(False)
    plt.show()
    #plt.savefig(os.path.join(project_path, 'Motif-UMAP', file, file+'.png'))
    plt.close()
...
def umap_vis_comm(file, embed, community_label, num_points):
    ...
    plt.gca().set_aspect('equal', 'datalim')
    plt.grid(False)
    plt.savefig('UMAP-community'+ file+'.png')
    plt.close()
    

def visualization(config, type= 'UMAP',label=None):
    ...
    for idx, file in enumerate(files):
        ...
        try:
            ...
            if type == 'TSNE' :
                reducer = TSNE(n_components=2, learning_rate='auto', init='random', perplexity=200, random_state=42)
            ...
            if type == 'TSNE' :
                np.save(os.path.join(path_to_file,"community","tsne_embedding_"+file+'.npy'), embed)
        ...
        if label == 'motif':
            ...
            umap_label_vis(file, cfg['project_path'], embed, motif_label, n_cluster, num_points)
