...
def get_labels(cfg, files, model_name, n_cluster):
    labels = []
    for file in files:
        path_to_file = os.path.join(cfg['project_path'],"results",file,model_name,'hmm-'+str(n_cluster),"")
...
def create_community_bag(files, labels, transition_matrices, cut_tree, n_cluster):
    ...
    communities_all = []
    for i, file in enumerate(files):
        #motif_usage = np.unique(labels[i], return_counts=True)
        lab = labels[i]
        usage = get_motif_usage(lab)
        print(usage)
...
def umap_vis(cfg, file, embed, community_labels_all):
    ...
    plt.grid(False)
    #plt.savefig('community-umap'+file+'.png')
    plt.close()
...
def community(config, show_umap=False, cut_tree=None):
    ...
    for idx, file in enumerate(files):
        ...
        if show_umap == True:
            ...
            umap_vis(cfg, file, embed, community_labels_all[idx])
