...
def embedd_latent_vectors_all(cfg, files, model, fixed) :
    ...
    for file in files:
        ...
        remove_idx = []
        brkpnts = np.load(os.path.join(project_path, 'results', file, file+'_breakpoints.npy'))
        ...
        print('hello')
        data = np.delete(data, remove_idx, axis=1)
        all_data.append(data)
        
    all_data = np.concatenate(all_data, axis=1)
    with torch.no_grad(): 
        for i in tqdm.tqdm(range(all_data.shape[1] - temp_win)):
        # for i in tqdm.tqdm(range(10000)):
            data_sample_np = all_data[:,i:temp_win+i].T
...
def same_parameterization(cfg, files, latent_vector_files, states, parameterization, all):
    ...
    if not all : 
        ...
        latent_vector_cat = latent_vector_files
    ...
    if parameterization == "kmeans":
        ...
        if cfg['hmm_trained'] == False:
            print("Using a Gaussian HMM for clustering!")
    ...
    if parameterization == "hmm" :
        np.save(os.path.join(cfg['project_path'], 'results', 'covariance.npy'), hmm_model.covars_)
        np.save(os.path.join(cfg['project_path'], 'results', 'hmm_labels.npy'), label)
        np.save(os.path.join(cfg['project_path'], 'results', 'transition_mat.npy'), hmm_model.transmat_)
        
    if all : 
        if parameterization == "kmeans":
            cluster_centers.append(clust_center)
        labels = label
        motif_usages = get_motif_usage(label)
...
def individual_parameterization(cfg, files, latent_vector_files, cluster, parameterization):
    random_state = cfg['random_state_kmeans']
    n_init = cfg['n_init_kmeans']
    ...
    if parameterization == 'hmm' :
        print("Using a HMM as parameterization for each file!")
        for i, file in enumerate(files) :
            hmm_model = vhmm.VariationalGaussianHMM(n_components=cluster, covariance_type="full", n_iter=100)
            hmm_model.fit(latent_vector_files[i])
            label = hmm_model.predict(latent_vector_files[i])
...
def pose_segmentation(config, all=False):
    ...
    if legacy == True:
        ...
        if not os.path.exists(os.path.join(cfg['project_path'],"results",file,model_name, parameterization+'-'+str(n_cluster),"")):
            ...
            if all : 
                latent_vectors = embedd_latent_vectors_all(cfg, files, model, fixed)
            else :
            ...
            if ind_param == False:
                ...
                labels, cluster_center, motif_usages = same_parameterization(cfg, files, latent_vectors, n_cluster, parameterization, all)
            else:
                print("Individual parameterization of latent vectors for %d cluster" %n_cluster)
                labels, cluster_center, motif_usages = individual_parameterization(cfg, files, latent_vectors, n_cluster, parameterization)
        ...
        if new == True:
            if all :
                np.save(os.path.join(cfg['project_path'],'results', 'latent_space_vector.npy'), latent_vectors)
                np.save(os.path.join(cfg['project_path'],'results','_km_label.npy'), labels)
                np.save(os.path.join(cfg['project_path'],'results','motif_usage.npy'), motif_usages)
                if parameterization=="kmeans":
                    np.save(os.path.join(cfg['project_path'],'results','cluster_center.npy'), cluster_center)
