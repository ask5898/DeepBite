...
from .vame_utils import filter_trajectories
...
def alignment(path_to_file, filename, pose_ref_index, video_format, crop_size, confidence, test, use_video=False, check_video=False):
    
    #read out data
    if test :
        data = pd.read_csv(os.path.join(path_to_file, 'testing', filename), skiprows=2)
    else :
    ...
    #print(np.array_equal(pose_list,p))
...
def egocentric_alignment(config, pose_ref_index=[0,5], crop_size=(300,300), use_video=False, video_format='.mp4', check_video=False, test=False):
    """ Happy aligning """
    #config parameters
    ...
    if test :
        ...
        egocentric_time_series, frames = alignment(path_to_file, file, pose_ref_index, video_format, crop_size, 
                                                    confidence, use_video=use_video, check_video=check_video, test=test)
        
        
        np.save(os.path.join(path_to_file,'testing','egocentric',file[:-3]+'-EGOCENTRIC.npy'), egocentric_time_series)
        ...
        for file in filename:
            print("Aligning data %s, Pose confidence value: %.2f" %(file, confidence))
            egocentric_time_series, frames = alignment(path_to_file, file, pose_ref_index, video_format, crop_size, 
                                                    confidence, use_video=use_video, check_video=check_video, test=False)
